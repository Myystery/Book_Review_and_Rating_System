    ...
    public function score($sentence)
    {
        ...
        foreach ($this->classes as $class) {
            ...
            foreach ($tokens as $token) {
                ...
                if (strlen($token) > $this->minTokenLength && strlen($token) < $this->maxTokenLength && ! in_array($token,
                        $this->ignoreList)) {
                    ...
                    //Score[class] is calculated by $scores[class] x $count +1 divided by the $classTokCounts[class] + $tokCount
                    $scores[$class] *= ($count + 1);
                }
            }

            //Score for this class is the prior probability multiplied by the score for this class
